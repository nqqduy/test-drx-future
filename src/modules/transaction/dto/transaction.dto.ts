...
export class TransactionHistoryDto extends PaginationDto {
  ...
  @ApiProperty({ required: false, description: 'Asset transaction', example: 'USDT' })
  @IsOptional()
  @IsString()
  asset: string;

  @ApiProperty({ required: false, description: 'Type of transaction', enum: TransactionType, example: 'DEPOSIT' })
  @IsOptional()
  @IsString()
  type: string;
