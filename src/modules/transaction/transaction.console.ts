...
export class TransactionConsole {
  ...
  private async sendTransactions(transactions: TransactionEntity[], producer: Producer): Promise<void> {
    if (transactions.length > 0) {
      ...
      await producer.send({ topic: KafkaTopics.matching_engine_input, messages });
