...
export class CreateOrderDto {
  @ApiProperty({ description: 'Side of order', name: 'side', enum: OrderSide, example: OrderSide.BUY })
  @IsNotEmpty()
  ...
  @ApiProperty({ required: true, description: 'Symbol of contract, get from /api/v1/instruments', example: 'BTCUSDT' })
  @IsString()
  @IsNotEmpty()
  symbol: string;

  @ApiProperty({ required: true, description: 'Type of order, LIMIT or MARKET', enum: ['LIMIT', 'MARKET'] })
  @IsNotEmpty()
  @IsIn(Object.keys(OrderType))
  type: OrderType;

  @ApiProperty({ required: true, description: 'Quantity of order', example: '1' })
  @IsNotEmpty()
  @IsPositiveBigNumber()
  @IsNotHaveSpace('quantity')
  quantity: string;

  @ApiProperty({ required: true, description: 'Price of order', example: '29200' })
  @ValidateIf((_object, value) => !!value)
  @IsPositiveBigNumber()
  @IsNotHaveSpace('price')
  price: string;

  @ApiProperty({ required: true, description: 'Equal to quantity', example: '1' })
  ...
  @ApiProperty({ description: 'Price take profit', required: false, example: '29500' })
  @ValidateIf((_object, value) => !!value)
  @IsPositiveBigNumber()
  @IsNotHaveSpace('takeProfit')
  takeProfit: string;

  @ApiProperty({ description: 'Price stop loss', required: false, example: '29000' })
  @ValidateIf((_object, value) => !!value)
  @IsPositiveBigNumber()
  @IsNotHaveSpace('stopLoss')
  stopLoss: string;

  @ApiProperty({ description: 'Trigger type', enum: [OrderTrigger.LAST, OrderTrigger.ORACLE] })
  @ValidateIf((_object, value) => !!value)
  ...
  @ApiProperty({ description: 'Callback rate of trailing stop order', required: false, example: '1' })
  @ValidateIf((_object, value) => !!value)
  @IsPositiveBigNumber()
  @IsNotHaveSpace('callbackRate')
  callbackRate: string;

  @ApiProperty({ description: 'Activation price of trailing stop order', required: false, example: '29500' })
  @ValidateIf((_object, value) => !!value)
  @IsPositiveBigNumber()
  ...
  @ApiProperty({ description: 'asset of order, map with symbol', enum: Object.keys(AssetOrder) })
  @ValidateIf((_object, value) => !!value)
