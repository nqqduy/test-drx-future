...
import { BalanceValidStatus, DexRunningChain, DexTransactionStatus } from 'src/modules/dex/dex.constant';
...
export class HealthConsole {
  ...
  private async putLatestSignatureMetrics(): Promise<void> {
    ...
    for (const service of keys) {
      ...
      const serviceSignatureId = await this.getSignatureId(latestBlock?.signature || '');
