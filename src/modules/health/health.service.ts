...
export class HealthService {
  ...
  async getHealth(): Promise<Record<string, unknown>> {
    ...
    const check = (): Promise<string> => {
      return new Promise<string>((resolve, _reject) => {
        redisClient.set('health', `${new Date().getTime()}`, (err, data) => {
          if (err) {
            throw new HttpException('Failed check health redis', HttpStatus.INTERNAL_SERVER_ERROR);
          } else if (data) {
    ...
    try {
      ...
      this.logger.error(e);
      throw new HttpException('Failed check health mysql', HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    try {
      ...
      this.logger.error(e);
      throw new HttpException('Failed check health kafka', HttpStatus.INTERNAL_SERVER_ERROR);
    }
