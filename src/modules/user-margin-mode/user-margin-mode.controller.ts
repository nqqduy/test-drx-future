...
export class UserMarginModeController {
  ...
  async updateMarginMode(@Body() input: UpdateMarginModeDto, @UserID() userId: number) {
    return {
  ...
  @Get()
  @UseGuards(JwtAuthGuard)
  async getMarginMode(@UserID() userId: number, @Query('instrumentId') instrumentId: number) {
    return {
