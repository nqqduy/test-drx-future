...
import { FUNDING_PREFIX, FUNDING_TTL, NEXT_FUNDING } from 'src/modules/funding/funding.const';
...
export class FundingService {
  ...
  constructor(
    ...
    @InjectRepository(InstrumentRepository, 'master') private instrumentRepositoryMaster: InstrumentRepository,
    @InjectRepository(InstrumentRepository, 'report') private instrumentRepositoryReport: InstrumentRepository,
    ...
    @InjectRepository(MarketIndexRepository, 'report') private marketIndicesRepositoryReport: MarketIndexRepository,
  ...
  fundingRateCaculation(
    ...
    const premium =
      ((Math.max(0, impactBidPrice - indexPrice) - Math.max(0, indexPrice - impactAskPrice)) / indexPrice) * 100;
  ...
  caculatePriceImpact(asksOrBids: string[][], marginAmount: number, indexPrice: number): number {
    ...
    for (const element of asksOrBids) {
      ...
      const quantity =
        marginAmount < Number(element[0]) * Number(element[1]) ? Number(element[1]) : marginAmount / Number(element[0]);
  ...
  async getImpactPrice(symbol: string, price: number): Promise<ImpactPrice> {
    const leverageMargins = await this.leverageMarginService.findAllByContract(ContractType.USD_M);
    ...
    const result = leverageMargins.find(
      (lm) => lm.symbol === symbol && +lm.maxLeverage === Math.max(...listMaxLeverage),
    );
  ...
  async getImpactPriceCoinM(symbol: string, price: number) {
    const leverageMargins = await this.leverageMarginService.findAllByContract(ContractType.COIN_M);
    const listMaxLeverage = leverageMargins
      .map((lm) => {
        if (lm.symbol === symbol) {
          return +lm.maxLeverage;
    ...
    if (orderbook == undefined || orderbook == null || (orderbook.bids.length == 0 && orderbook.asks.length == 0)) {
      ...
      return {
        impactBidPrice: this.caculateCoinMImpactPrice(orderbook.bids, IMN_COINM.VALUE, price),
        impactAskPrice: price,
        interestRate: 0.01,
        maintainMargin: result.maintenanceMarginRate == null ? null : Number(result.maintenanceMarginRate),
      };
      ...
      return {
        impactBidPrice: this.caculatePriceImpact(orderbook.asks, IMN_COINM.VALUE, price),
        impactAskPrice: price,
        interestRate: 0.01,
        maintainMargin: result.maintenanceMarginRate == null ? null : Number(result.maintenanceMarginRate),
      };
    ...
    return {
      impactBidPrice: this.caculateCoinMImpactPrice(orderbook.bids, IMN_COINM.VALUE, price),
      impactAskPrice: this.caculateCoinMImpactPrice(orderbook.asks, IMN_COINM.VALUE, price),
      interestRate: 0.01,
      maintainMargin: result.maintenanceMarginRate == null ? null : Number(result.maintenanceMarginRate),
    };
  }
  ...
  async getMarketIndex(): Promise<MarketIndex[]> {
    // check cached data
    const lastInserted = await this.redisService.getClient().get(`${INDEX_PRICE_PREFIX}last_inserted`);
    ...
    const query =
      'SELECT `symbol`, `price` FROM `market_indices` AS m1 INNER JOIN (SELECT MAX(`id`) AS `latest` FROM `market_indices` GROUP BY `symbol`) AS m2 ON m1.id = m2.latest';
  ...
  async saveFundingRate(symbol: string, fundingRate: string): Promise<void> {
    ...
    await Promise.all([
      this.cacheManager.set(`${FUNDING_PREFIX}${symbol}`, +fundingRate, { ttl: FUNDING_TTL }),
      ...
      this.cacheManager.set(`${FUNDING_PREFIX}next_funding`, Date.now() + NEXT_FUNDING),
    ]);
  }

  async getNextFunding(symbol: string) {
    const nextFundingCache = await this.redisService.getClient().get(`${FUNDING_PREFIX}next_funding`);
    if (nextFundingCache) {
      ...
      const nextFundingDb = await this.fundingRepositoryReport
        ...
        .getRawOne();
      if (nextFundingDb) {
        await this.redisService.getClient().set(`${FUNDING_PREFIX}next_funding`, +nextFundingDb.nextFunding);
  ...
  async fundingRate(symbol: string) {
    try {
      const fundingRateCache = await this.cacheManager.get(`${FUNDING_PREFIX}${symbol}`);
      if (fundingRateCache) {
        ...
        if (fundingRateDb?.fundingRate) {
          this.cacheManager.set(`${FUNDING_PREFIX}${symbol}`, +fundingRateDb.fundingRate, { ttl: FUNDING_TTL });
  ...
  public async setLastPay(): Promise<void> {
    await this.redisService.getClient().set(`${FUNDING_PREFIX}last_pay`, Date.now());
  }

  public async setLastUpdate(): Promise<void> {
    await this.redisService.getClient().set(`${FUNDING_PREFIX}last_update`, Date.now());
  }

  public async getLastPay(): Promise<number | undefined> {
    const value = await this.redisService.getClient().get(`${FUNDING_PREFIX}last_pay`);
    return value ? Number(value) : 0;
  }

  public async getLastUpdate(): Promise<number | undefined> {
    const value = await this.redisService.getClient().get(`${FUNDING_PREFIX}last_update`);
    return value ? Number(value) : 0;
  }
  ...
  async getFundingRatesFromTo(symbol: string, { from, to }: FromToDto): Promise<FundingEntity[]> {
  ...
  async findHistoryBefore(date: Date): Promise<FundingHistoryEntity | undefined> {
    return await this.fundingHistoryRepositoryReport.findHistoryBefore(date);
  }

  async findHistoryBatch(fromId: number, count: number): Promise<FundingHistoryEntity[]> {
  ...
  async sendMailFundingFee(dataFundingRates: any[]) {
    ...
    for (const userSetting of userSettings) {
      ...
      if (symbols.length > 0 && userSetting.fundingFeeTrigger) {
        ...
        this.notificationService.genDataNotificationFirebase(NOTIFICATION_TYPE.FUNDING_FEE, userSetting.userId);
      }
    }
  }
}
