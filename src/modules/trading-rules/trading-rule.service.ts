...
import { TRADING_RULES_CACHE, TRADING_RULES_CACHE_NO_LIMIT, TRADING_RULES_TTL } from './trading-rules.constants';
...
export class TradingRulesService {
  ...
  async insertOrUpdateTradingRules(input: TradingRulesModeDto): Promise<TradingRulesEntity> {
    try {
      ...
      await Promise.all([
        this.cacheManager.set(TRADING_RULES_CACHE, tradingRuleCache, { ttl: TRADING_RULES_TTL }),
        this.cacheManager.set(TRADING_RULES_CACHE_NO_LIMIT, tradingRuleCache, { ttl: TRADING_RULES_TTL }),
      ]);
  ...
  async getAllTradingRules(input: PaginationDto) {
    const tradingRulesCache = (await this.cacheManager.get(TRADING_RULES_CACHE)) as any[];
    ...
    await this.cacheManager.set(TRADING_RULES_CACHE, tradingRules, { ttl: TRADING_RULES_TTL });
  ...
  async getTradingRuleByInstrumentId(symbol: string) {
    const tradingRulesCache = await this.cacheManager.get(`${TRADING_RULES_CACHE}_${symbol}`);
    if (tradingRulesCache) {
    ...
    await this.cacheManager.set(`${TRADING_RULES_CACHE}_${symbol}`, data);
    return data;
  }

  async getAllTradingRulesNoLimit(): Promise<TradingRulesEntity[]> {
    const tradingRulesCache = (await this.cacheManager.get(TRADING_RULES_CACHE_NO_LIMIT)) as TradingRulesEntity[];
    ...
    await this.cacheManager.set(TRADING_RULES_CACHE_NO_LIMIT, tradingRules, { ttl: TRADING_RULES_TTL });
    return tradingRules;
  }
}
