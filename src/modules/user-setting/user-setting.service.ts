...
import { NOTIFICATION_KEY, TP_SL_NOTIFICATION } from 'src/shares/enums/setting.enum';
...
export class UserSettingeService {
  ...
  async getUserSettingByKey(key: string, userId: number): Promise<UserSettingEntity> {
  ...
  async updateNotificationSetting(key: string, userId: number) {
    ...
    if (!userSetting) {
      throw new HttpException(httpErrors.USER_SETTING_NOT_FOUND, HttpStatus.NOT_FOUND);
    }
    const endTime = moment().utc().endOf('day').toDate().getTime();

    const startTime = userSetting.time.getTime();
    if (now <= endTime && userSetting.notificationQuantity === TP_SL_NOTIFICATION.QUANTITY) {
      throw new HttpException(httpErrors.USER_SETTING_TP_SL_NOTIFICATION, HttpStatus.BAD_REQUEST);
    }
