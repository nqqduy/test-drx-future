...
export class UserSettingController {
  ...
  async updateUserPreferenceSetting(
    ...
    return {
      data: await this.userSettingService.updateUserSettingByKey(UserSettingEntity.NOTIFICATION, body, userId),
    };
  }

  @Get('preference')
  @UseGuards(JwtAuthGuard)
  async getUserPreferenceSetting(@UserID() userId: number): Promise<ResponseDto<UserSettingEntity>> {
    const userSetting = await this.userSettingService.getUserSettingByKey(UserSettingEntity.NOTIFICATION, userId);
    return {
      data: userSetting,
    };
  }
