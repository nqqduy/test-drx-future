...
export class UserService {
  ...
  constructor(
    @InjectRepository(ApiKeyRepository, 'master') private apiKeyRepository: ApiKeyRepository,
    @InjectRepository(ApiKeyRepository, 'report') private apiKeyReportRepository: ApiKeyRepository,
    ...
    @InjectRepository(UserSettingRepository, 'master') private userSettingRepository: UserSettingRepository,
    ...
    @InjectRepository(CoinInfoRepository, 'master') private coinInfoRepository: CoinInfoRepository,
  ) {
    this.web3 = new Web3();
  }
  ...
  async getUserByAccountId(accountId: number): Promise<UserEntity> {
  ...
  async createUser(body: CreateUserDto): Promise<UserEntity> {
    ...
    const newUser = await this.usersRepositoryMaster.save({ ...body, status: UserStatus.DEACTIVE });
  ...
  async createUserWithoutChecking(
    ...
    @TransactionRepository(AccountEntity) transactionRepositoryAccount?: Repository<AccountEntity>,
  ): Promise<UserEntity> {
  ...
  async updateUserFavoriteMarket(
    ...
    if (!existSymbol) throw new HttpException(httpErrors.INSTRUMENT_DOES_NOT_EXIST, HttpStatus.BAD_REQUEST);
