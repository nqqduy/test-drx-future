...
export class CoinInfoService {
  ...
  async getInfo() {
    ...
    // ttl 5 hours
    await this.cacheManager.set(KEY_CACHE_HEALTHCHECK_GET_FUNDING, true, { ttl: 60 * 60 + 5 });
  }
  ...
  async getAllCoinInfo(): Promise<CoinInfoEntity[]> {
    ...
    await this.cacheManager.set(COIN_INFO_CACHE, coinInfo, { ttl: COIN_INFO_TTL });
    return coinInfo;
  }
  ...
  async getCurrentPriceWithBTC(symbol: string) {
    ...
    const coingeckoUrl = config.get<string>('coin_info.coingeckoUrl');
    const response = await fetch(
      `${coingeckoUrl}/coins/markets?vs_currency=btc&order=market_cap_desc&per_page=100&page=1&sparkline=false&locale=en&ids=${symbol}`,
      {
        method: 'get',
      },
    );
    const data = await response.json();
    const price = data.find((d) => d.symbol === symbol)?.current_price;
    if (!price) {
