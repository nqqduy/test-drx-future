...
export class MailService {
  ...
  async checkWaitTime(userId: number): Promise<string> {
    const isWait = await this.redisService.getClient().get(`${MailService.WAIT_PREFIX}${userId}`);
    if (isWait) {
  ...
  async getPendingEmail(userId: UserEntity): Promise<string> {
    const keys = await this.redisService.getClient().keys(`${MailService.MAIL_PREFIX}*`);
    if (keys.length == 0) return null;
    for (let i = keys.length - 1; i >= 0; i--) {
      const dto: UpdateEmailDto = JSON.parse(await this.redisService.getClient().get(keys[i]));
  ...
  async getQueueStats(): Promise<{ activeCount: number; failedCount: number; waitingCount: number }> {
  ...
  async sendMailFundingFee(email: string, fundingRate: string, symbols: string[]): Promise<void> {
    let phoneSupport = await this.cacheManager.get<string>(`${MAIL.PHONE_SUPPORT}`);
    let emailSupport = await this.cacheManager.get<string>(`${MAIL.EMAIL_SUPPORT}`);
    let bannerImage = await this.cacheManager.get<string>(`${MAIL.BANNER_IMAGE}`);
    let footerImage = await this.cacheManager.get<string>(`${MAIL.FOOTER_IMAGE}`);
    ...
    if (!phoneSupport || emailSupport) {
      ...
      console.log('url: ', url);
      const result = await lastValueFrom(this.httpService.get(url).pipe(map((response) => response.data)));
      if (!result.data) return;
      if (result.data.contact_phone) {
        await this.cacheManager.set(<string>`${MAIL.PHONE_SUPPORT}`, result.data.contact_phone, {
          ttl: Number.MAX_SAFE_INTEGER,
        });
        phoneSupport = result.data.contact_phone;
      }
      if (result.data.contact_email) {
        await this.cacheManager.set(<string>`${MAIL.EMAIL_SUPPORT}`, result.data.contact_email, {
          ttl: Number.MAX_SAFE_INTEGER,
        });
    ...
    if (!bannerImage || !footerImage) {
      ...
      const result = await lastValueFrom(this.httpService.get(url).pipe(map((response) => response.data)));
      if (!result.data) return;
      await this.cacheManager.set(<string>`${MAIL.BANNER_IMAGE}`, result.data.header, {
        ttl: Number.MAX_SAFE_INTEGER,
      });
      await this.cacheManager.set(<string>`${MAIL.FOOTER_IMAGE}`, result.data.footer, {
        ttl: Number.MAX_SAFE_INTEGER,
      });
  ...
  async sendMailWhenTpSlOrderTriggered(command: CommandOutput) {
    for (const order of command.orders) {
      ...
      if (conditionSendMailTrigger) {
        try {
          const { userNotificationSettings, user } = await this.userSettingService.getUserSettingByUserId(
            order.userId as number,
          );
          if (!userNotificationSettings || !user) {
            continue;
          }
          if (Number(userNotificationSettings.notificationQuantity) >= TP_SL_NOTIFICATION.QUANTITY) {
            continue;
          }
          if (!userNotificationSettings.stopLossTrigger) {
            continue;
          }
          const trigger = order.trigger === OrderTrigger.ORACLE ? 'Mark' : 'Last';
          let phoneSupport = await this.cacheManager.get<string>(`${MAIL.PHONE_SUPPORT}`);
          let emailSupport = await this.cacheManager.get<string>(`${MAIL.EMAIL_SUPPORT}`);
