...
export class AccountController {
  ...
  async getAccountByUserId(@UserID() userId: number): Promise<ResponseDto<AccountEntity>> {
    return {
  ...
  @Post('/withdraw')
  @UseGuards(JwtAuthGuard)
  async withdrawal(@UserID() ownerId: number, @Body() withdrawDto: WithdrawalDto): Promise<ResponseDto<unknown>> {
    return {
  ...
  @Get('/balance/:symbol')
  @UseGuards(JwtAuthGuard)
  async getBalance(@UserID() userId: number, @Param('symbol') symbol: string): Promise<BalanceDto> {
