...
export class AccessTokenService {
  ...
  async removeAccessToken(token: string, userId: number): Promise<{ message: string }> {
    const isExist: AccessToken = await this.accessTokenRepoMaster.findOne({
      where: { token, userId },
    });
    if (!isExist) {
      throw new HttpException(httpErrors.ACCESS_TOKEN_NOT_FOUND, HttpStatus.NOT_FOUND);
    }
