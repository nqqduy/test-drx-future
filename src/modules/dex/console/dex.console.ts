...
export class DexConsole {
  constructor(private logger: Logger, private dexService: DexService, public readonly kafkaClient: KafkaClient) {
  ...
  async dexActions(): Promise<void> {
    ...
    await consumer.subscribe({ topic: KafkaTopics.matching_engine_output, fromBeginning: false });
    await consumer.run({
      eachMessage: async ({ topic, message }) => {
        const commands = JSON.parse(message.value.toString()) as CommandOutput[];
