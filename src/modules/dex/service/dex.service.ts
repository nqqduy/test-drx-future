...
import { CommandCode, CommandOutput } from 'src/modules/matching-engine/matching-engine.const';
...
export class DexService {
  ...
  async saveDexActions(offset: string, commands: CommandOutput[]): Promise<InsertResult | null> {
    const dexActions = [];
    for (const command of commands) {
      if (command.code === CommandCode.WITHDRAW && command.data.status === TransactionStatus.APPROVED) {
        dexActions.push(await this._withdrawalToDexAction(offset, command.data));
        continue;
      }
      if (command.code === CommandCode.PAY_FUNDING && command.fundingHistories) {
  ...
  private async _fundingsToDexActions(offset, fundingHistories) {
    ...
    for (const fundingHistory of fundingHistories) {
      const dexParameter = {
        ...
        amount: utils.parseUnits(fundingHistory.amount.toString(), defaultScale).toString(),
      };
      dexActions.push({
  ...
  private async _withdrawalToDexAction(offset, data) {
    const dexParameter = {
      ...
      amount: utils.parseUnits(data.amount.toString(), collateralDecimal).toString(),
      fee: utils.parseUnits((data.fee || 0).toString(), collateralDecimal).toString(),
    };
  ...
  private async _tradesToDexActions(offset: string, trades: any) {
    ...
    for (const trade of trades) {
      ...
      const dexParameter = {
        ...
        quantity: utils.parseUnits(trade.quantity.toString(), defaultScale).toString(),
        price: utils.parseUnits(trade.price.toString(), defaultScale).toString(),
        bankruptPrice: bankruptPrice.toString(),
        liquidationSide,
        buyerFee: utils.parseUnits(trade.buyFee.toString(), defaultScale).toString(),
        sellerFee: utils.parseUnits(trade.sellFee.toString(), defaultScale).toString(),
  ...
  async handlePickDexActions() {
    ...
    while (true) {
      ...
      const sentRecords = await this.dexActionTransactionRepo.find({
        select: ['id'],
        where: { status: In([DexTransactionStatus.PENDING, DexTransactionStatus.SENT]) },
  ...
  async handleVerifyDexActions() {
    while (true) {
      ...
      const receipts = await Promise.all(
        sentRecords.map(async (record) => ({ id: record.id, data: await provider.getTransactionReceipt(record.txid) })),
      );
  ...
  async handleHistoryDexActions() {
    ...
    const queryTopics = [utils.id('UpdateMargin(uint8,uint64,uint64,address,int128,int128)')];
        ...
        const histories = await Promise.all(
          events.map(async (event) => {
            ...
            return {
              ...
              oldMargin: utils.formatUnits(event.args.oldMargin, defaultScale).toString(),
              newMargin: utils.formatUnits(event.args.newMargin, defaultScale).toString(),
            };
          }),
        );
