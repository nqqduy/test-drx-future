...
export class SotaDexWrapper {
  ...
  async getMemberAccount(address: PublicKey): Promise<[PublicKey, number]> {
    return PublicKey.findProgramAddress(
      [SOTADEX_MEMBER_SEED, this.dexId.toArrayLike(Buffer, 'le', 8), address.toBuffer()],
      this.program.programId,
    );
  }

  async getUsdcTokenAccount(address: PublicKey) {
    return Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, this.usdcId, address);
  }

  async getMemberPositionAccount(address: PublicKey, instructionId: BN): Promise<[PublicKey, number]> {
    return PublicKey.findProgramAddress(
      [
